...
public final class Genesis
{
    ...
    { //decide how and what to respond, return if we should keep the program alive
        if(s.trim().equals(""))
            return true; //nothing to do, but keep the program alive
        String response = "";
        if(last == null)
        { //first message must always be a greeting
            boolean newg = true;
            ...
            boolean notg = true;
            for(String r : fm.getResponses(ResponseType.GREETING))
            { //check if THE LAST MESSAGE is another greeting
                if(transform(last).equalsIgnoreCase(transform(r)))
                    notg = false;
            }
            boolean notf = true;
            for(String r : fm.getResponses(ResponseType.FAREWELL))
            { //check if they're saying a known farewell
                if(transform(s).equalsIgnoreCase(transform(r)))
                    notf = false;
            }
            if((!notf || s.equalsIgnoreCase("exit")) && notg)
            { //if they're doing a farewell or saying "exit", and THE LAST MESSAGE is not a greeting
                boolean newf = true;
                for(String r : fm.getResponses(ResponseType.FAREWELL))
                { //check if it's a new farewell
                    if(transform(last).equalsIgnoreCase(transform(r)))
                        newf = false;
                }
        ...
        if(!containsLaugh)
        { //if super serious business mode is on
            ...
            { //it's not math
                ...
                { //the user is telling us information
    ...
    { //join an array together with a specified string in between starting at a specified offset
        String s = set[offset];
        int i = 0;
        for(String part : set)
        {
    ...
    { //reverse between 1st and 3rd person, so Genesis makes sense
    ...
    { //solve math expressions
        ...
        if(m.matches()) { //if this is a correct math expression
            ...
            { //checking for valid operations
                ...
                switch(m.group(2))
                { //check the operation symbol and do math
                    default:
                        break;
                    case "+":
...
**/
    ...
    { //add random punctuation, and capitalize the first character
        return cap(s) + punc();
    }
    ...
    { //super hacky way of combining a PrintWriter and a PrintStream

        private PrintWriter w;
        private PrintStream s;

        PrintWriterStream(PrintWriter w)
        { //support for PrintWriter
            if(w == null)
                throw new NullPointerException();
            this.w = w;
        }

        PrintWriterStream(PrintStream s)
        { //support for PrintStream
            if(s == null)
                throw new NullPointerException();
            this.s = s;
        }
